...
An AI-powered application that categorizes incoming emails and automatically generates professional replies in real time. Built using Python, FastAPI, PyTorch/Transformers, Scikit-learn, and Streamlit.
...
A TF-IDF + Logistic Regression classifier predicts the intent of an email — Action Required, Meeting, Question, or Informational.
A Transformer-based generative model (PyTorch / Hugging Face transformers) drafts a context-aware reply, with an option to generate multiple distinct suggestions.
A FastAPI backend serves both models via REST endpoints, and a Streamlit frontend provides an interactive UI for pasting an email and viewing the predicted category, confidence score, and generated reply.
System Architecture
flowchart TD
